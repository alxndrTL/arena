"""
Adapted from https://github.com/nanowell/AdEMAMix-Optimizer-Pytorch
"""
...
class AdEMAMix(Optimizer):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999, 0.9999), eps=1e-8, weight_decay=0, alpha=5.0, T_alpha_beta3=None):
        assert len(betas)==3, "Ademamix requieres 3 beta values"
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                if p.grad is not None:
                    ...
                    if len(state) == 0:
                        state['step'] = 0
                        state['exp_avg'] = torch.zeros_like(p, memory_format=torch.preserve_format) # first moment m1 (beta1)
                        state['exp_avg_sq'] = torch.zeros_like(p, memory_format=torch.preserve_format) # second moment v (beta2)
                        state['exp_avg_slow'] = torch.zeros_like(p, memory_format=torch.preserve_format) # first moment m2 (beta3)
    ...
    def _update_adamemix(self, params, grads, exp_avgs, exp_avg_sqs, exp_avg_slow, state_steps, beta1, beta2, beta3, alpha, T_alpha_beta3, lr, weight_decay, eps):
