...
class Mamba2Config:
    ...
    max_len: int = 1024 # not actually used
...
class Mamba2(nn.Module):
    ...
    def forward(self, x, caches=None, seq_pos=0):
        ...
        for i, layer in enumerate(self.layers):
            x, c = layer(x, caches[i] if caches is not None else None) # (B, L, D)
...
class ResidualBlock(nn.Module):
    def __init__(self, config: Mamba2Config):
        ...
        self.norm = RMSNorm(self.config.d_model, self.config.norm_eps, self.config.mup)
    ...
    def get_empty_cache(self, batch_size):
        ssm_state = torch.zeros(batch_size, self.config.n_heads, self.config.d_head, self.config.d_state, device=self.mixer.in_proj.weight.device, dtype=self.mixer.in_proj.weight.dtype)
        conv_state = torch.zeros(batch_size, self.mixer.conv1d.weight.shape[0], self.config.d_conv, device=self.mixer.conv1d.weight.device, dtype=self.mixer.conv1d.weight.dtype)
...
class Mamba2Block(nn.Module):
    ...
    def forward(self, u, cache=None, seq_idx=None):
        ...
        if self.config.use_mem_eff_path:
            ...
            if return_cache:
                # get h_cache (ie ssm_state) from output
                out, h_cache = out

                # compute conv_cache (ie conv_state) with last d_conv xBC
    ...
    def step(self, x, cache):
        ...
        if causal_conv1d_update is None:
            ...
            xBC = causal_conv1d_update(xBC, conv_state, rearrange(self.conv1d.weight, "d 1 w -> d w"), self.conv1d.bias, self.config.activation)
        ...
        if selective_state_update is None:
            ...
            # compute output
            y = torch.einsum("bhpn,bn->bhp", ssm_state.to(x.dtype), C)
            ...
            y = selective_state_update(ssm_state, x_reshaped, dt, A, B, C, D, z=None, dt_bias=dt_bias, dt_softplus=True)
            y = rearrange(y, "b h p -> b (h p)")
        ...
        out = self.out_proj(y)
        return out.unsqueeze(1), (ssm_state, conv_state)
